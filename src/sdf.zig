...
pub const SystemDescription = struct {
    ...
    pub const CNode = struct {
        allocator: Allocator,
        name: []const u8,
        remaining_untypeds: bool,
        size_bits: u8,

        pub fn create(allocator: Allocator, name: []const u8, remaining_untypeds: bool, size_bits: u8) CNode {
            return CNode {
                .allocator = allocator,
                .name = allocator.dupe(u8, name) catch @panic("Could not dupe CNode name"),
                .remaining_untypeds = remaining_untypeds,
                .size_bits = size_bits,
            };
        }
        ...
        pub fn render(cnode: *const CNode, writer: ArrayList(u8).Writer, separator: []const u8) !void {
            ...
            if (cnode.remaining_untypeds) {
                try std.fmt.format(writer, " remaining_untypeds=\"true\"", .{});
            }
