...
fn addPassthroughDeviceMapping(system: *Self, name: []const u8, device: *dtb.Node, device_reg: [][2]u128, index: usize) void {
    ...
    if (device_mr == null) {
        device_mr = Mr.physical(system.allocator, system.sdf, mr_name, device_size, false, .{
            .paddr = device_paddr,
        });
...
pub fn connect(system: *Self) !void {
    ...
    if (sdf.arch.isArm()) {
        ...
        if (gic.hasMmioCpuInterface()) {
            ...
            if (gic_vcpu_mr == null) {
                gic_vcpu_mr = Mr.physical(allocator, sdf, gic_vcpu_mr_name, gic.vcpu_size.?, false, .{ .paddr = gic.vcpu_paddr.? });
        ...
        const guest_ram_mr = blk: {
            if (system.one_to_one_ram) {
                break :blk Mr.physical(allocator, sdf, guest_mr_name, guest_ram_size, false, .{ .paddr = memory_paddr });
            } else {
                break :blk Mr.create(allocator, guest_mr_name, guest_ram_size, false, .{});
            }
        };
