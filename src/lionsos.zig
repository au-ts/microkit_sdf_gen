...
pub const FileSystem = struct {
    ...
    pub fn connect(system: *FileSystem, options: ConnectOptions) void {
        ...
        const fs_command_queue = Mr.create(allocator, fmt(allocator, "fs_{s}_command_queue", .{fs.name}), system.command_queue_size, false, .{});
        const fs_completion_queue = Mr.create(allocator, fmt(allocator, "fs_{s}_completion_queue", .{fs.name}), system.completion_queue_size, false, .{});
        ...
        const fs_share = blk: {
            if (system.data_mr) |data_mr| {
                ...
                const mr = Mr.create(allocator, fmt(allocator, "fs_{s}_share", .{fs.name}), system.data_size, false, .{});
    ...
    pub const Fat = struct {
        ...
        pub fn connect(fat: *Fat) !void {
            ...
            const stack1 = Mr.create(allocator, fmt(allocator, "{s}_stack1", .{fs_pd.name}), 0x40_000, false, .{});
            const stack2 = Mr.create(allocator, fmt(allocator, "{s}_stack2", .{fs_pd.name}), 0x40_000, false, .{});
            const stack3 = Mr.create(allocator, fmt(allocator, "{s}_stack3", .{fs_pd.name}), 0x40_000, false, .{});
            const stack4 = Mr.create(allocator, fmt(allocator, "{s}_stack4", .{fs_pd.name}), 0x40_000, false, .{});
    ...
    pub const VmFs = struct {
        ...
        pub fn connect(vmfs: *VmFs) !void {
            ...
            const config_share_region = Mr.create(allocator, fmt(allocator, "fs_{s}_guest_conf_share", .{vmfs.fs_vm_sys.guest.name}), config_share_size, false, .{});
