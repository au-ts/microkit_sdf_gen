...
pub const FileSystem = struct {
    ...
    pub fn connect(system: *FileSystem, options: ConnectOptions) void {
        ...
        const fs_command_queue = Mr.create(allocator, fmt(allocator, "fs_{s}_command_queue", true, .{fs.name}), system.command_queue_size, .{});
        const fs_completion_queue = Mr.create(allocator, fmt(allocator, "fs_{s}_completion_queue", true, .{fs.name}), system.completion_queue_size, .{});
    ...
    pub const VmFs = struct {
        ...
        pub fn connect(vmfs: *VmFs) !void {
            ...
            const config_share_region = Mr.create(allocator, fmt(allocator, "fs_{s}_guest_conf_share", true, .{vmfs.fs_vm_sys.guest.name}), config_share_size, .{});
