...
pub const I2c = struct {
    ...
    pub fn connectDriver(system: *I2c) void {
        ...
        const mr_req = Mr.create(allocator, "i2c_driver_request", system.region_req_size, false, .{});
        const mr_resp = Mr.create(allocator, "i2c_driver_response", system.region_resp_size, false, .{});
    ...
    pub fn connectClient(system: *I2c, client: *Pd, i: usize) void {
        ...
        const mr_req = Mr.create(allocator, fmt(allocator, "i2c_client_request_{s}", .{client.name}), system.region_req_size, false, .{});
        const mr_resp = Mr.create(allocator, fmt(allocator, "i2c_client_response_{s}", .{client.name}), system.region_resp_size, false, .{});
        const mr_data = Mr.create(allocator, fmt(allocator, "i2c_client_data_{s}", .{client.name}), system.region_data_size, false, .{});
